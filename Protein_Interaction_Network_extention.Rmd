...
breast = read.csv("Breast_related_genes.csv")
prost = read.csv("Prost_related_genes.csv")
eso = read.csv("Eso_related_genes.csv")
head = read.csv("Head_related_genes.csv")

biliary = read.csv("Biliary_related_genes.csv")
bone = read.csv("Bone_related_genes.csv")
cns = read.csv("CNS_related_genes.csv")
kidney = read.csv("Kidney_related_genes.csv")
liver = read.csv("Liver_related_genes.csv")
lymph = read.csv("Lymph_related_genes.csv")
myeloid = read.csv("Myeloid_related_genes.csv")
ovary = read.csv("Ovary_related_genes.csv")
panc = read.csv("Panc_related_genes.csv")
skin = read.csv("Skin_related_genes.csv")
stomach = read.csv("Stomach_related_genes.csv")
CNS_bone_myeloid = read.csv("CNS_bone_myeloid_related_genes.csv")
...
data_mapped_CNS_bone_myeloid <- string_db$map(CNS_bone_myeloid, my_data_frame_id_col_names = "SYMBOL", removeUnmappedRows = TRUE)
```
...
string_db$plot_network(data_mapped_CNS_bone_myeloid$STRING_id)
```
...
inter_score_df_CNS_bone_myeloid = string_db$get_interactions(data_mapped_CNS_bone_myeloid$STRING_id)
...
links_stomach = inter_score_df_stomch %>% 
  ...
  distinct(from, to, combined_score, .keep_all=TRUE) %>% 
  rename("combined_score" = "weight")
links_CNS_bone_myeloid = inter_score_df_CNS_bone_myeloid %>% 
  mutate(from=data_mapped_CNS_bone_myeloid[match(from, data_mapped_CNS_bone_myeloid$STRING_id), "SYMBOL"]) %>% 
  mutate(to=data_mapped_CNS_bone_myeloid[match(to, data_mapped_CNS_bone_myeloid$STRING_id), "SYMBOL"]) %>% 
  distinct(from, to, combined_score, .keep_all=TRUE) %>% 
  rename("combined_score" = "weight")
...
nodes_CNS_bone_myeloid = links_CNS_bone_myeloid %>% 
  {data.frame(CNS_bone_myeloid=c(.$from, .$to))} %>% 
  distinct()
...
PPInet_CNS_bone_myeloid = igraph::graph_from_data_frame(d=links_CNS_bone_myeloid, vertices=nodes_CNS_bone_myeloid, directed=FALSE)
```
...
igraph::V(PPInet_CNS_bone_myeloid)$deg <- igraph::degree(PPInet_CNS_bone_myeloid)
igraph::V(PPInet_CNS_bone_myeloid)$size <- igraph::degree(PPInet_CNS_bone_myeloid)/5
igraph::E(PPInet_CNS_bone_myeloid)$width <- igraph::E(PPInet_CNS_bone_myeloid)$weight/10
```
```{r}
...
```{r}
# Breast
ggraph(PPInet_breast,layout = "linear", circular = TRUE) + geom_edge_arc(aes(filter=weight<=850, edge_width=width), color = "lightblue", show.legend = FALSE) + geom_edge_arc(aes(filter=weight>850, edge_width=width, label=weight), color = "pink", show.legend = FALSE) + geom_node_point(aes(size=size), color="red", alpha=0.7) + geom_node_text(aes(label=name), size = 5, repel = FALSE) + scale_edge_width(range = c(0.1, 3)) + scale_size_continuous(range = c(1,10) ) + guides(size=F) + theme_graph()
```
```{r}
hpaVis(visType="Patho",targetGene = c("TP53", "RB1", "PTEN", "PIK3CA", "EGFR", "SMARCA4"), targetCancer = c("breast cancer"))
```
```{r}
...
```{r}
# Prostate
ggraph(PPInet_prost,layout = "linear", circular = TRUE) + geom_edge_arc(aes(filter=weight<=850, edge_width=width), color = "lightblue", show.legend = FALSE) + geom_edge_arc(aes(filter=weight>850, edge_width=width, label=weight), color = "pink", show.legend = FALSE) + geom_node_point(aes(size=size), color="red", alpha=0.7) + geom_node_text(aes(label=name), size = 5, repel = FALSE) + scale_edge_width(range = c(0.1, 3)) + scale_size_continuous(range = c(1,10) ) + guides(size=F) + theme_graph()
```
```{r}
...
```
```{r}
hpaVisTissue(targetGene = c("PTEN", "CDKN2A", "PIK3CA", "RB1", "TP53", "EGFR"), targetTissue = c("esophagus", "oral mucosa", "salivary gland", "tongue"))
```

```{r}
# Head
ggraph(PPInet_head,layout = "linear", circular = TRUE) + geom_edge_arc(aes(filter=weight<=850, edge_width=width), color = "lightblue", show.legend = FALSE) + geom_edge_arc(aes(filter=weight>850, edge_width=width, label=weight), color = "pink", show.legend = FALSE) + geom_node_point(aes(size=size), color="red", alpha=0.7) + geom_node_text(aes(label=name), size = 5, repel = FALSE) + scale_edge_width(range = c(0.1, 3)) + scale_size_continuous(range = c(1,10) ) + guides(size=F) + theme_graph()
```
```{r}
...
```{r}
# Biliary
ggraph(PPInet_biliary,layout = "linear", circular = TRUE) + geom_edge_arc(aes(filter=weight<=850, edge_width=width), color = "lightblue", show.legend = FALSE) + geom_edge_arc(aes(filter=weight>850, edge_width=width, label=weight), color = "pink", show.legend = FALSE) + geom_node_point(aes(size=size), color="red", alpha=0.7) + geom_node_text(aes(label=name), size = 5, repel = FALSE) + scale_edge_width(range = c(0.1, 3)) + scale_size_continuous(range = c(1,10) ) + guides(size=F) + theme_graph()
```
...
```{r}
# Liver
ggraph(PPInet_liver,layout = "linear", circular = TRUE) + geom_edge_arc(aes(filter=weight<=850, edge_width=width), color = "lightblue", show.legend = FALSE) + geom_edge_arc(aes(filter=weight>850, edge_width=width, label=weight), color = "pink", show.legend = FALSE) + geom_node_point(aes(size=size), color="red", alpha=0.7) + geom_node_text(aes(label=name), size = 5, repel = FALSE) + scale_edge_width(range = c(0.1, 3)) + scale_size_continuous(range = c(1,10) ) + guides(size=F) + theme_graph()
```
```{r}
...
```{r}
# Ovary
ggraph(PPInet_ovary,layout = "linear", circular = TRUE) + geom_edge_arc(aes(filter=weight<=850, edge_width=width), color = "lightblue", show.legend = FALSE) + geom_edge_arc(aes(filter=weight>850, edge_width=width, label=weight), color = "pink", show.legend = FALSE) + geom_node_point(aes(size=size), color="red", alpha=0.7) + geom_node_text(aes(label=name), size = 5, repel = FALSE) + scale_edge_width(range = c(0.1, 3)) + scale_size_continuous(range = c(1,10) ) + guides(size=F) + theme_graph()
```
```{r}
...
```
```{r}
hpaVis(visType="Patho",targetGene = c("KRAS", "KMT2C", "PIK3CA", "RB1", "TP53", "EGFR", "ATM", "BRAF", "ARID1A", "CDKN2A", "MTOR", "PTEN", "GRIN2A", "CTNNB1", "CASP8", "NF1", "SMARCA4"), targetCancer = c("pancreatic cancer"))
```
...
```
```{r}
hpaVis(visType="Patho",targetGene = c("MSH6", "KEAP1", "KMT2C", "RB1", "TP53", "EGFR", "ATM", "FBXW7", "BRAF", "KMT2D", "CDKN2A", "MTOR", "PTEN", "NRAS", "NFE2L2", "SMARCA4"), targetCancer = c("skin cancer"))
```

```{r}
# Stomach
ggraph(PPInet_stomach,layout = "linear", circular = TRUE) + geom_edge_arc(aes(filter=weight<=850, edge_width=width), color = "lightblue", show.legend = FALSE) + geom_edge_arc(aes(filter=weight>850, edge_width=width, label=weight), color = "pink", show.legend = FALSE) + geom_node_point(aes(size=size), color="red", alpha=0.7) + geom_node_text(aes(label=name), size = 5, repel = FALSE) + scale_edge_width(range = c(0.1, 3)) + scale_size_continuous(range = c(1,10) ) + guides(size=F) + theme_graph()
```
```{r}
...
```{r}
# CNS&bone&myeloid
ggraph(PPInet_CNS_bone_myeloid,layout = "linear", circular = TRUE) + geom_edge_arc(aes(filter=weight<=850, edge_width=width), color = "lightblue", show.legend = FALSE) + geom_edge_arc(aes(filter=weight>850, edge_width=width, label=weight), color = "pink", show.legend = FALSE) + geom_node_point(aes(size=size), color="red", alpha=0.7) + geom_node_text(aes(label=name), size = 5, repel = FALSE) + scale_edge_width(range = c(0.1, 3)) + scale_size_continuous(range = c(1,10) ) + guides(size=F) + theme_graph()
```

```{r}
hpaVisTissue(targetGene = c("KEAP1", "KMT2C", "TP53", "EGFR", "ATM", "FBXW7", "KMT2D", "CTNNB1", "NF1", "NRAS", "GRIN2A", "SMARCA4"), targetTissue = c("bone marrow"))
```
